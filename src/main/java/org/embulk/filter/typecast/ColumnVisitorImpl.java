        ...
        implements ColumnVisitor
{
    ...
    private final HashSet<String> shouldCastJsonRecursivelySet = new HashSet<>();
        ...
        for (ColumnConfig columnConfig : task.getColumns()) {
            ...
            for (int i = 1; i < parts.length; i++) {
                if (parts[i].contains("[")) {
                    ...
                    this.shouldCastJsonRecursivelySet.add(partialPath.toString());
    ...
    private boolean shouldCastJsonRecursively(String jsonPath)
    {
        return shouldCastJsonRecursivelySet.contains(jsonPath);
    }

    private Value castJsonRecursively(PluginTask task, String jsonPath, Value value)
    {
        if (!shouldCastJsonRecursively(jsonPath)) {
            return value;
        }
